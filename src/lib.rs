...
Implementing your own low-level drawing logic is not always easy, so *repaint* also provides a set of common algorithms that can be used to implement the drawing logic for your canvas. As an example, if you are implementing your own canvas on an image and you don't know how to draw a line, you can use the `Bresenham` algorithm to do it, but it could be tedious to implement it yourself, so *repaint* provides some common algorithms that can be used to implement the drawing logic for your canvas; in this case, you might want to use the [`base::rasterization::algorithms::line`] module and pick the algorithm that suits your needs.
