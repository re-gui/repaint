...
pub fn clip_rect(rect: &FRect, clip_rect: &FRect) -> Option<FRect> {
    ...
    Some(FRect {
        min: Vec2f::new(rect.min.x.max(clip_rect.min.x), rect.min.y.max(clip_rect.min.y)),
        max: Vec2f::new(rect.max.x.min(clip_rect.max.x), rect.max.y.min(clip_rect.max.y)),
    })
}
...
pub fn clip_line(start: &Vector2<f32>, end: &Vector2<f32>, rect: &FRect) -> Option<(Vector2<f32>, Vector2<f32>)> {
    ...
    if start == end {
        if
            (start.x < rect.min.x) || (start.x > rect.max.x) ||
            (start.y < rect.min.y) || (start.y > rect.max.y)
        {
            return None;
        }
    }
    ...
    if
        ((start.x < rect.min.x) && (end.x < rect.min.x)) ||
    ...
    if
        ((start.y < rect.min.y) && (end.y < rect.min.y)) ||
