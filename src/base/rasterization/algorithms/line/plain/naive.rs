...
use crate::base::{clipping::clip_line, defs::rect::{FRect, Rect}};
...
pub fn line<F: FnMut(u32, u32) -> ()>(start: &Vec2f, end: &Vec2f, plot: &mut F, rect: &Rect<u32>) -> () {
    ...
    if let Some((clipped_start, clipped_end)) = clip_line(&start, &end, &f_rect) {
        let sanitize_vec = |v: Vec2f| -> Vec2u {
            Vec2u::new(
                (v.x.max(0.0).round() as u32).max(rect.min.x).min(rect.max.x),
                (v.y.max(0.0).round() as u32).max(rect.min.y).min(rect.max.y)
            )
        };
        ...
        if !steep {
            ...
            for x in (start.x as u32)..(end.x as u32) {
                let y = (start.y as f32) + (x as f32 - start.x as f32) * (delta.y as f32) / (delta.x as f32);
            ...
            for y in (start.y as u32)..(end.y as u32) {
                let x = (start.x as f32) + (y as f32 - start.y as f32) * (delta.x as f32) / (delta.y as f32);
